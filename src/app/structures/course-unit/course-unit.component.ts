...
export class CourseUnitComponent implements OnInit {
     ...
     ngOnInit(): void {
       this.generalForm = this.fb.group({
        ...
        description: [''], 
        creditHours: ['', [Validators.required, Validators.minLength(1)]], 
    ...
    this.sub = this.route.paramMap.subscribe(
      params => {
        const id1 = params.get('courseUnitCode');
       // console.log('id1 ' + id1);

        this.generalId = params.get('courseUnitCode');
     ...
     display(generalInterface: CourseUnit ): void{
       if(this.generalForm){
         //resets the form
         this.generalForm.reset();
       }
       ...
       if(this.generalId ==='0'){
         ...
         this.pageTitle = `Edit Details: ${generalInterface[0].courseUnitCode}`;
     ...
     deleteOne(): void{
      const deleteObject = { ...this.generalObjectInterface, ...this.generalForm.value};
       if(deleteObject.courseUnitCode === 0 ) {
         // Don't delete, 
         alert('Deleting is not possible');
         this.onSaveComplete();
       } else {
         if(confirm(`Are sure you want to Delete: ${deleteObject.courseUnitCode}`)){
           this.generalService.deleteOne(deleteObject.courseUnitCode)
           .subscribe({
             next: () => this.onSaveComplete(),
            // error: err => this.errorMessage = err
           });
